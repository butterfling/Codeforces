...
// vector<int> getLongestSubarray(vector<int> &arr){
...
//     int completeCycles = (N - 1) / 8;
//     int remainder = (N - 1) % 8;

//     int number = completeCycles * 15;
...
void addMatrix(const vector<vector<int>>&A,const vector<vector<int>>&B,int r, int c, int i, int j){

    if(i==r) return;

    if(j==c){
        cout<<nline;
        addMatrix(A,B,r,c,i+1,0);
    }
    else{
        int sum = A[i][j]+B[i][j];
        cout<<sum<<" ";
        addMatrix(A,B,r,c,i,j+1);
    }
}

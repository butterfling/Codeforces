...
void solve() {
    ...
    vector<long long int> roomsInEachDorm(n);

    for(int i=0;i<n;i++){
        cin>>roomsInEachDorm[i];
    }

    vector<long long int> letters(m);
    for(int i=0;i<m;i++){
        cin>>letters[i];
    }
    ...
    pref[0]=roomsInEachDorm[0];

    for(int i=1;i<n;i++){
        pref[i]=pref[i-1]+roomsInEachDorm[i];
    }

    for(int i=0;i<m;i++){
        
        auto it = lower_bound(pref.begin(), pref.end(),letters[i]);

        long long int ans = distance(pref.begin(),it);

        long long int dormNumber = (pref[ans]-letters[i]);
        cout<<ans+1<<" "<<roomsInEachDorm[ans]-dormNumber<<nline;
    }
